...
#include <cassert>
#include <string>
#include <functional> // per std::equal_to
#include "set.hpp"

// =================================================================
// TIPO CUSTOM E FUNTORE
// =================================================================
...
// Necessario per operator<< e save su file
...
struct IsLongString
{
    bool operator()(const std::string &s) const { return s.length() > 3; }
};
...
void test_range_constructor_raw_pointer(); // Sostituisce vector
void test_rule_of_three();
void test_iterators_and_const();
void test_math_operators_edge_cases();
void test_filter_predicate();
void test_stress_reallocation(); // Test importante per la tua implementazione array
void test_files();
...
void test_custom_type()
{
    ...
    {
        if (PointEqual()(s[i], p1))
            found_p1 = true;
        if (PointEqual()(s[i], p2))
            found_p2 = true;
    }
